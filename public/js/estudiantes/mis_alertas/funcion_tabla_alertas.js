...
function initTablaMisAlertas() {
    ...
    const filtroTipo = document.getElementById('filtro-tipo-alertas');
    const filtroEstado = document.getElementById('filtro-estado-alertas');
    ...
    const cfg = {
        ...
        urlVer: tabla.dataset.urlVerMiAlerta,
    };
    ...
    function renderRows(items) {
        if (!items || items.length === 0) {
            tbody.innerHTML = `<tr><td colspan="8" class="td-estado-tabla">Sin alertas para mostrar.</td></tr>`;
            return;
        }
            ...
            <tr>
                ...
                <td>
                    <div class="botones-tabla">
                        <button type="button" class="btn-ver-detalles-item btn-ver-mi-alerta" data-id="${it.id}">
    ...
    async function cargarTabla(page = 1) {
        ...
        try {
            ...
            console.error(err);
            displayMensajeToast('<p class="error">Error de conexión al cargar tus alertas.</p>');
        }
    }
    ...
    tabla.addEventListener('click', async (ev) => {
        const btnVer = ev.target.closest('.btn-ver-mi-alerta');
        if (!btnVer) return;
            ...
            <div class="scroll-contenido-modal-detalles">
                <div class="contenedor-detalles">
                    <div class="caja-detalle">
                        ...
                        <p class="info-detalle">${escapeHtml(d.tipo_alerta_texto || '—')}</p>
                    </div>
