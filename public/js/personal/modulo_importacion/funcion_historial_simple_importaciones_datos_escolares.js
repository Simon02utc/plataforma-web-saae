...
function initTablaHistorialImportaciones() {
    const tabla = document.getElementById('tabla-historial-simple-importaciones-datos-escolares');
    const btnRefrescar = document.getElementById('btn-refrescar-tabla-historial-simple-datos-escolares');
    ...
    const cfgTabHistorialSimpleImportacionesDatosEscolares = {
        urlListadoHistorialSimpleImportacionesDatosEscolares: tabla.dataset.urlTablaListadoHistorialSimpleImportacionesDatosEscolares,
        urlVerDetallesImportacionDatosEscolaresSimple: tabla.dataset.urlTablaVerDetallesImportacionDatosEscolaresSimple,
    ...
    function renderRows(items) {
        ...
        tbody.innerHTML = items.map((it) => `
            <tr>
                ...
                <td class="td-fuente-importacion">${escapeHtml(it.fuente_datos_escolares ?? '—')}</td>
    ...
    async function cargarHistorial(limit = cfgTabHistorialSimpleImportacionesDatosEscolares.limitePorDefecto) {
        ...
        try {
            const safeLimit = Math.max(
                1,
                Math.min(Number(limit) || cfgTabHistorialSimpleImportacionesDatosEscolares.limitePorDefecto, cfgTabHistorialSimpleImportacionesDatosEscolares.limiteMaximo)
            );

            const url = new URL(cfgTabHistorialSimpleImportacionesDatosEscolares.urlListadoHistorialSimpleImportacionesDatosEscolares, window.location.origin);
    ...
    async function verDetalleImportacion(id) {
        try {
            const r = await fetchJson(buildUrl(cfgTabHistorialSimpleImportacionesDatosEscolares.urlVerDetallesImportacionDatosEscolaresSimple, id));
            if (!r.ok) return;
            ...
            displayModalDetalles(`
                ...
                <div class="scroll-contenido-modal-detalles">
                    ...
                    <div class="contenedor-detalles">
                        ...
                        <div class="caja-detalle">
                            <p class="nombre-detalle">Parser de la fuente de datos:</p>
    ...
    async function descargarArchivoImportacion(id) {
        try {
            const url = buildUrl(cfgTabHistorialSimpleImportacionesDatosEscolares.urlDescargarArchivoImportacionDatosEscolaresSimple, id);
    ...
    btnRefrescar.addEventListener('click', async () => {
        ...
        await cargarHistorial(cfgTabHistorialSimpleImportacionesDatosEscolares.limitePorDefecto);
        setLoadingTabla(btnRefrescar, false);
    });
    //==========================================================

    cargarHistorial(cfgTabHistorialSimpleImportacionesDatosEscolares.limitePorDefecto);
}
//==========================================================