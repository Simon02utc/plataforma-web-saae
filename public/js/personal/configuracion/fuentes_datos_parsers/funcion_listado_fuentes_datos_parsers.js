...
//======TABLA DE FUENTES DE DATOS(LISTA, VER, EDITAR Y ELIMINAR)======
function initTablaFuentesDatos() {
  const tabla = document.getElementById('tabla-listado-fuentes-datos');
  const btnRefrescar = document.getElementById('btn-refrescar-tabla-fuentes-datos');
  const inputBuscar = document.getElementById('input-buscar-fuentes-datos');
  const btnBuscar = document.getElementById('btn-buscar-fuentes-datos');
  ...
  const cfgTabFuentes = {
    urlListadoFuentesDatos: tabla.dataset.urlTablaListadoFuentesDatos,
    urlVerParsersFuenteDatos: tabla.dataset.urlTablaVerParsersFuenteDatos,
    urlVerFuenteDatos: tabla.dataset.urlTablaVerFuenteDatos,
    urlEditarFuenteDatos: tabla.dataset.urlTablaEditarFuenteDatos,
    urlEliminarFuenteDatos: tabla.dataset.urlTablaEliminarFuenteDatos,
  };

  let aborter = null;


  //PINTADO DE LA TABLA DE FUENTES DE DATOS
  function renderRows(items) {
    if (!items || items.length === 0) {
      const texto = inputBuscar.value.trim()
        ? 'No se encontraron fuentes de datos con esa búsqueda.'
        : 'Sin fuentes de datos registradas.';
    ...
    tbody.innerHTML = items.map((it) => {
      ...
      const estadoTexto = it.estado_fuente ? 
      ...
      return `  
        <tr>
          ...
          <td class="td-nombre">${escapeHtml(it.nombre_fuente || '—')}</td>
          <td class="td-descripcion">${escapeHtml(it.descripcion_fuente || '—')}</td>
          ...
          <td>
            <div class="botones-tabla">
              <button type="button" class="btn-editar-item btn-editar-fuente-datos" data-id="${it.id}">
          ...
          <td>
            <div class="botones-tabla">
              <button type="button" class="btn-eliminar-item btn-eliminar-fuente-datos" data-id="${it.id}">
  ...
  //CARGA DEL PINTADO DE LA TABLA Y LA FUNCION DE BUSCAR FUENTES DE DATOS
  async function cargar() {
    ...
    try {
      const url = new URL(cfgTabFuentes.urlListadoFuentesDatos, window.location.origin);
      ...
      console.error(err);
      displayMensajeToast('<p class="error">Error de conexión al cargar las fuentes de datos.</p>');
    }
  }


  //VER PARSERS DE LA FUENTE
  async function verDetalle(id) {
    try {
      const r = await fetchJson(buildUrl(cfgTabFuentes.urlVerParsersFuenteDatos, id));
      if (!r.ok) return;

      const d = r.data?.data;
      if (!d) {
        displayMensajeToast('<p class="error">No se pudo obtener la información de los parsers de la fuente de datos.</p>')
        return;
      }

      const parsersHtml = (d.parsers_de_fuente_datos && d.parsers_de_fuente_datos.length)
        ? `<ul>
          ${d.parsers_de_fuente_datos.map(p => `
            <li>${escapeHtml(p.nombre_parser)} <span class="informacion-extra-li-modal-detalles">(${escapeHtml(p.clave)})</span></li>
          `).join('')}
        </ul>`
        : '<p class="info-detalle">Esta fuente no tiene un parser asignado.</p>';

      displayModalDetalles(`
        <h3 class="titulo-modal-detalles">Parser de la fuente de datos #${d.id}</h3>

          <div class="scroll-contenido-modal-detalles">

              <div class="contenedor-detalles">
                <div class="caja-detalle">
                  <p class="nombre-detalle">Fuente:</p>
                  <p class="info-detalle">${escapeHtml(d.nombre_fuente_datos || '—')}</p>
                </div>

                <div class="caja-detalle">
                  <p class="nombre-detalle">Descripcion:</p>
                  <p class="info-detalle">${escapeHtml(d.descripcion_fuente_datos || '—')}</p>
                </div>
      ...
      console.error(err);
      displayMensajeToast('<p class="error">Error de conexión al ver los parsers de la fuente de datos.</p>');
    }
  }


  //FORMULARIO DE EDITAR FUENTE DE DATOS
  async function editarFuente(id) {
    try {
      const r = await fetchJson(buildUrl(cfgTabFuentes.urlVerFuenteDatos, id));
      if (!r.ok) return;

      const d = r.data?.data;
      if (!d) {
        displayMensajeToast('<p class="error">No se pudo obtener la información de la fuente datos.</p>');
        return;
      }


      const activoChecked = d.estado_fuente_datos ? 'checked' : '';
      ...
      displayModalFormularioEditar(`
        <form id="form-editar-fuente-datos" class="form-editar-contenido">
          
          <div class="form-title">
              <span>Editar fuente de datos #${d.id}</span>
          </div>
          ...
          <div class="form-inputs">
            <div class="scroll-editar">
              ...
              <div class="input-box-editar">
                ...
                <input class="input-field" type="text" name="nombre" pattern="^[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+(?:\\s+[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+)*$" title="Solo letras y espacios. Ej: Fuente Concentrado de Estudiantes" value="${escapeHtml(d.nombre_fuente_datos || '')}" autocomplete="off" autocapitalize="words" disabled  required>
              </div>

              <div class="input-box-editar">
                <label class="nombre-input">Descripción:</label>
                <textarea class="input-field" name="descripcion" maxlength="500" title="Solo letras, espacios, y signos de puntuacion" autocomplete="off" disabled>${escapeHtml(d.descripcion_fuente_datos || '')}</textarea>
              </div>
              ...
              <div class="input-box">
                  <label class="input-field switch-estado">
                      ...
                      <span class="texto-switch-estado">Activar fuente</span>
                  </label>
              </div>
      ...
      const form = document.getElementById('form-editar-fuente-datos');
      if (!form) return;


      //====================HABILITAR EDICION====================
      //En los <input>, <textarea> y el <input type="checkbox"> tienen "disabled"
      const btnEditarFuenteDatos = form.querySelector('.btn-editar-form');
      const btnCancelarEdicionFuenteDatos = form.querySelector('.btn-cancelar-borrar');
      const btnGuardarEdicionFuenteDatos = form.querySelector('.btn-guardar-enviar');

      btnEditarFuenteDatos?.addEventListener('click', () => {
        ...
        btnEditarFuenteDatos.style.display = 'none';
        btnCancelarEdicionFuenteDatos.style.display = 'inline-flex';
        btnGuardarEdicionFuenteDatos.style.display = 'inline-flex';

        form.querySelector('[name="nombre"]')?.focus();//Enfocarse a ese input
      });

      //=====================CANCELAR EDICION=====================
      btnCancelarEdicionFuenteDatos?.addEventListener('click', () => {
          form.reset();
          ...
          btnEditarFuenteDatos.style.display = 'inline-flex';
          btnCancelarEdicionFuenteDatos.style.display = 'none';
          btnGuardarEdicionFuenteDatos.style.display = 'none';
      });
      //==========================================================
      ...
      form.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          const rr = await fetchJson(buildUrl(cfgTabFuentes.urlEditarFuenteDatos, id), {
            method: 'PUT',
            body: payload,
          });
          ...
          if (typeof displayMensajeToast === 'function') {
            ...
            displayMensajeToast(`<p class="exito">${escapeHtml(rr.data?.message || 'Fuente de datos actualizadoacorrectamente.')}</p>`);
          }

        } catch (error) {
          console.error(error);
          displayMensajeToast(`<p class="error">Error de conexión al actualizar la fuente de datos. <span class="ext">Revisa tu servidor o recarga la página.</span></p>`);
        } finally {
      ...
      console.error(err);
      displayMensajeToast('<p class="error">Error de conexión al editar la fuente de datos.</p>');
    }
  }


  //ELIMIANR UNA FUENTE DE DATOS
  async function eliminarFuente(id, button) {

    // modal de confirmacion
    const ok = await modalConfirmarAccion({
        titulo: 'Eliminar fuente de datos',
        mensaje: `
            <p>¿Seguro que quieres eliminar la fuente de datos #${id}?</p>
            <ul>
                <li>Esta acción eliminará el registro de la fuente de datos.</li>
                <li>Si hay personal que ocupa esta fuente, podría causar problemas en el modulo de importación.</li>
                <li>Si la fuente tienen registros de importaciones, no se podra eliminar debido a que se utiliza en auditoría.</li>
                <li>Se recomienda el desactivar la fuente de datos.</li>
    ...
    try {
      ...
      const r = await fetchJson(buildUrl(cfgTabFuentes.urlEliminarFuenteDatos, id), {
        method: 'DELETE',
      });
      ...
      await cargar();
      if (typeof displayMensajeToast === 'function') {
        displayMensajeToast(`<p class="exito">${escapeHtml(r.data?.message || 'Fuente de datos eliminada correctamente.')}</p>`);
      }
    } catch (err) {
      console.error(err);
      displayMensajeToast('<p class="error">Error de conexión al eliminar la fuente de datos.</p>');
    } finally {
  ...
  tabla.addEventListener('click', (ev) => {
    ...
    const btnEditar = ev.target.closest('.btn-editar-fuente-datos');
    if (btnEditar) {
      editarFuente(btnEditar.dataset.id);
      return;
    }

    const btnEliminar = ev.target.closest('.btn-eliminar-fuente-datos');
    if (btnEliminar) {
      eliminarFuente(btnEliminar.dataset.id, btnEliminar);
    }
  });
          ...
          <div class="form-inputs">
            <div class="scroll-editar">
              ...
              <div class="input-box-editar">
                ...
                <input class="input-field" type="text" name="clave" pattern="^[a-z]+(?:_[a-z]+)*$" title="Solo letras minúsculas y guion bajo '_' como separador. Ejemplo: parser_fuente_concentrado_estudiantes_xlsx" value="${escapeHtml(d.clave || '')}" autocomplete="off" autocapitalize="none" disabled required>
              </div>

              <div class="input-box-editar">
                <label class="nombre-input">Nombre:</label>
                <input class="input-field" type="text" name="nombre" pattern="^[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+(?:\\s+[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+)*$" title="Solo letras y espacios. Ej: Parser de fuente concentrado de estudiantes xlsx" value="${escapeHtml(d.nombre || '')}" autocomplete="off" autocapitalize="words" disabled required>
              </div>
    ...
    const ok = await modalConfirmarAccion({
        ...
        mensaje: `
            ...
            <ul>
                ...
                <li>Si hay una o varias fuentes usandolo, podría causar graves problemas en el modulo de importación de datos escolares.</li>
                <li>Si la fuente tienen registros de importaciones, no se podra eliminar debido a que se utiliza en auditoría.</li>
                <li>Los parsers son de suma importancia para las fuentes de datos, ya que con ellos se extraen los datos de su archivo.</li>
                <li>Se recomienda el desactivar el parser.</li>
