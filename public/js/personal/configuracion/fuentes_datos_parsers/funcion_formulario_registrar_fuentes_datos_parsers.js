...
/*--------------------FUNCION REGISTRAR RELOJES------------------------*/
    const personalForm = document.getElementById('form-registrar-fuente-datos');
    if (personalForm) {
        personalForm.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                displayMensajeToast(`<p class="exito">${data?.message ?? 'Fuente de datos registrado correctamente.'}</p>`);
                clearForm('form-registrar-fuente-datos');
                setTimeout(() => location.reload(), 3000); //Recargar la pagina 3segundos despues de recibir Exito

            } catch (error) {
                console.error(error);
                displayMensajeToast(`<p class="error">Error de conexión al registrar la fuente de datos. Revisa tu servidor o recarga la página.</p>`);
            } finally {
    ...
    if (roleForm) {
        roleForm.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                console.error(error);
                displayMensajeToast(`<p class="error">Error de conexión al registrar el parser. Revisa tu servidor o recarga la página.</p>`);
            } finally {
                setLoadingFormulario(button, false);
            }
        });
