...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const estudiantesForm = document.getElementById('form-registrar-estudiante');
    if (estudiantesForm) {
        estudiantesForm.addEventListener('submit', async (event) => {
            event.preventDefault();

            const formData = new FormData(estudiantesForm);
            const button = estudiantesForm.querySelector('.btn-guardar-enviar');
            ...
            try {
                const response = await fetch(estudiantesForm.action, {
                    method: 'POST',
                    headers: {
                ...
                clearForm('form-registrar-estudiante');
