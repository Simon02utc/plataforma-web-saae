...
class ParserRelojChecadorNuevoPlantillaArchivoXlsx implements AsistenciaParserInterface
{
    /**
     *  ============================================================
     *  PLANTILLA BASE PARA NUEVOS PARSERS XLSX
     *  ============================================================
     *
     *  ESTA CLASE ES UNA BASE REUTILIZABLE
     *
     *  ¿Qué debes adaptar cuando llegue un reloj nuevo?
     *  1) buscarHojaTurnos() y/o buscarHojaAsistencia()
     *  2) leerPeriodoDesdeTurnos()
     *  3) mapearColumnasTurnos()
     *  4) mapearColumnasAsistencia()
     *  5) leerTurnos()
     *  6) leerAsistencia()
     *
     *  ¿Qué NO debes cambiar si no es necesario?
     *  - La estructura del return final
     *  - El contrato de parsear()
     *  - El manejo general por tipo de importación
     *  - Los helpers genéricos de normalización
     *
     *  Esta plantilla asume:
     *  - Archivo Excel
     *  - Puede haber hoja de turnos, asistencia o ambas
     *  - El servicio espera una salida uniforme
     */
    public function parsear(
        ...
        if ($tipoImportacion === 'COMPLETA') {
            if (!$wsTurnos) {
                throw new \RuntimeException('La importación COMPLETA requiere la hoja de "turnos".');
            }

            if (!$wsAsistencia) {
                throw new \RuntimeException('La importación COMPLETA requiere la hoja de "asistencia".');
            }
            ...
            $this->validarMapaDias('Turnos', $metaTurnos, $diasPeriodo, $advertencias);
            $this->validarMapaDias('Asistencia', $metaAsis, $diasPeriodo, $advertencias);
            ...
            if ($filasTurnosSinNumeroControl > 0) {
                $advertencias[] = "Se omitieron {$filasTurnosSinNumeroControl} fila(s) de \"turnos\" porque no tenían número de control.";
            }
            ...
            if (!$wsAsistencia) {
                throw new \RuntimeException('La importación SOLO_ASISTENCIA requiere la hoja de asistencia.');
            }
            ...
            if (!$wsTurnos) {
                throw new \RuntimeException('La importación SOLO_TURNOS requiere la hoja de "turnos".');
            }
            ...
            if ($filasTurnosSinNumeroControl > 0) {
                $advertencias[] = "Se omitieron {$filasTurnosSinNumeroControl} fila(s) en \"turnos\" porque no tenían número de control.";
            }
    ...
    // ============================================================
    // METODOS A ADAPTAR SEGUN EL FORMATO DEL NUEVO RELOJ
    // ============================================================
    ...
    protected function validarMapaDias(
        ...
        if (!empty($inesperadas)) {
            ...
            $advertencias[] =
                "En la hoja \"{$nombreHoja}\" se ignoró la(s) columna(s) del día no reconocidas: " .
                implode(', ', $cols) . '.';
        }
    }
    ...
    // ========== HELPERS ==========
    protected function buscarHojaOpcional($spreadsheet, string $busqueda): ?Worksheet
    {
